...
3. **Picker**: Open selection UI (default when no bang or target)
   ...
   :TermUpdate
   ```
   **Note:** If only one terminal is available, the picker is skipped and that terminal is selected automatically
...
Open terminal picker or focus a specific terminal if only one matches.
...
```vim
" Open terminal picker or focus if only one terminal
:TermSelect
