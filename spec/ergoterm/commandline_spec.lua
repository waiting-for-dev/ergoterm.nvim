...
describe("commandline.term_new_complete", function()
  it("completes available options", function()
    ...
    assert.is_true(vim.tbl_contains(result, "cleanup_on_job_exit="))
  end)
  ...
  it("completes boolean values for cleanup_on_job_exit", function()
    local result = commandline.term_new_complete("cleanup_on_job_exit=", "cleanup_on_job_exit=", 18)

    assert.is_true(vim.tbl_contains(result, "cleanup_on_job_exit=true"))
    assert.is_true(vim.tbl_contains(result, "cleanup_on_job_exit=false"))
  end)
end)
